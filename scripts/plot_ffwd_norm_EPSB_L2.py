...
def run_forward_pass(model: LightningModule, datamodule: LightningDataModule):
    ...
    with torch.no_grad():
        ...
        for i, act in enumerate(activities):
            norm = torch.linalg.norm(act).item()
            print(f"Layer {i}: shape={act.shape}, Mean L2 Norm={norm:.4f}")
        print("----------------------\n")
    return activities

@hydra.main(version_base="1.3", config_path="../configs", config_name="experiment/ffwd_norm_epb.yaml")
def main(cfg: DictConfig) -> None:
    """Main function to plot the mean L2 norm of feedforward pass activities vs. network depth for different strategies and initializations."""
    ...
    for strategy_info in strategies_to_test:
        for init_method_dict in initializations_to_test:
            ...
            l2_norms_at_l1 = []
            l2_norms_at_l_quarter = []
            l2_norms_at_l_half = []
            l2_norms_at_l_three_quarter = []
            l2_norms_at_l_full = []
            ...
            for depth in cfg.analysis.depths:
                ...
                for seed in range(cfg.analysis.n_seeds):
                    ...
                    if len(all_layer_activities) > layer_idx_l1:
                        ...
                        norms_for_current_depth["l1"].append(torch.linalg.norm(activity).item())
                    if len(all_layer_activities) > layer_idx_l_quarter:
                        activity = all_layer_activities[layer_idx_l_quarter]
                        norms_for_current_depth["l_quarter"].append(torch.linalg.norm(activity).item())
                    if len(all_layer_activities) > layer_idx_l_half:
                        activity = all_layer_activities[layer_idx_l_half]
                        norms_for_current_depth["l_half"].append(torch.linalg.norm(activity).item())
                    if len(all_layer_activities) > layer_idx_l_three_quarter:
                        activity = all_layer_activities[layer_idx_l_three_quarter]
                        norms_for_current_depth["l_three_quarter"].append(torch.linalg.norm(activity).item())
                    if len(all_layer_activities) > layer_idx_l_full:
                        activity = all_layer_activities[layer_idx_l_full]
                        norms_for_current_depth["l_full"].append(torch.linalg.norm(activity).item())
                
                mean_l2 = np.mean(norms_for_current_depth["l1"]) if norms_for_current_depth["l1"] else np.nan
                ...
                l2_norms_at_l1.append(mean_l2)
                l2_norms_at_l_quarter.append(mean_l_quarter)
                l2_norms_at_l_half.append(mean_l_half)
                l2_norms_at_l_three_quarter.append(mean_l_three_quarter)
                l2_norms_at_l_full.append(mean_l_full)

                print(f"        Mean L2 Norms for Depth {depth}:\n" \
                      f"            l=1: {mean_l2:.4f}\n" \
            ...
            plt.plot(cfg.analysis.depths, l2_norms_at_l1, marker='o', linestyle='-', label='l=1')
            plt.plot(cfg.analysis.depths, l2_norms_at_l_quarter, marker='x', linestyle='--', label='l=L/4')
            plt.plot(cfg.analysis.depths, l2_norms_at_l_half, marker='s', linestyle='-', label='l=L/2')
            plt.plot(cfg.analysis.depths, l2_norms_at_l_three_quarter, marker='D', linestyle='--', label='l=3L/4')
            plt.plot(cfg.analysis.depths, l2_norms_at_l_full, marker='^', linestyle='-', label='l=L')

            # --- Finalize Plot for the current combination ---
            plt.xlabel("Network Depth (L)", fontsize=14)
            plt.ylabel("Mean L2 Norm of Activity", fontsize=14)
            ...
            save_dir = f"plotting/ffwd_activity_norm/EPSB_strategies_L2/"
